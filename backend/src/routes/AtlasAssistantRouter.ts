  ...
  static getRouter(): Router {
    if (!this.router) {
      ...
      this.router.get('/models', (req: Request, res: Response) =>
        this.getAtlasModels(req, res),
      );
      this.router.get('/latest_messages', (req: Request, res: Response) =>
        this.getLatestMessages(req, res),
      );
    }
    return this.router;
  }
  ...
  static async sendTextToAtlas(req: Request, res: Response) {
    try {
      ...
      const response = await axios.post(
        ...
        {
          headers: {
            Authorization: selectedModel.authToken
              ? `Bearer ${selectedModel.authToken}`
              : undefined,
