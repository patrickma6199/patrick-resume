...
const AssistantMainContent: React.FC = () => {
    const [assistantLastMessage, setAssistantLastMessage] =
        useState<string>('');
    ...
    const speak = (text: string) => {
        ...
        utterance.voice = window.speechSynthesis
            .getVoices()
            .filter(voice => voice.name === 'Google UK English')?.[0];
    ...
    const handleCompleteMessage = () => {
        handleInputEnd(async spokenText => {
            ...
            const response = await API.sendTextToAtlas(
                enteredPasskey,
                spokenText,
            );
            if (!response.success) {
                alert(
                    response.response.message ||
                        'Unexpected Error Occurred: Failed to send message to Atlas Assistant',
                );
                return;
            }
        ...
        <div className="relative flex flex-col w-full h-[] items-center px-0 gap-8 box-border lg:px-40 overflow-x-hidden">
            <div className="w-full h-full flex flex-col justify-center items-center box-border z-1 gap-10">
                {!browserSupportsSpeechRecognition || !isMicrophoneAvailable ? (
                    <h1 className="w-[50%]">
                        Sorry, speech recognition is not supported in your
                        browser or your microphone is not enabled.
                    </h1>
                ) : (
                    <>
                        <motion.img
